...
function renderHeadpics(){
  $('tbody').innerHTML=teams.map((t,i)=>`<tr><td>${esc(t.no)}</td><td><strong>${esc(t.team)}</strong></td><td>${t.file?`<div class="avatar-preview"><img class="preview" src="${t.file.url}" alt=""></div>`:`<label class="row-file">Ch&#7885;n logo<input type="file" accept="image/png,image/jpeg,image/webp" onchange="pickForTeam(${i},this.files[0])"></label>`}</td><td><div class="headpics-picker"><select class="headpics-select"><option value="">Ch&#7885;n HEADPICS...</option>${HEADPICS.map(([name,id])=>`<option value="${id}" ${t.id===id?'selected':''}>${name} &mdash; ${id}</option>`).join('')}</select><button class="headpics-ok" onclick="setHeadpics(${i},this.previousElementSibling.value)">OK</button></div></td><td>${t.file?`<span style="color:#67e66f">&#272;&#227; gh&#233;p t&#7921; &#273;&#7897;ng &middot; ${esc(t.avatar||t.id)}</span>`:'<span style="color:#f5c451">Ch&#432;a c&#243; logo</span>'}</td><td>${t.file?`<button onclick="downloadOne(${i})">T&#7843;i PNG</button> <button onclick="detach(${i})">B&#7887;</button>`:''}</td></tr>`).join('');
  const free=logos.filter(f=>!teams.some(t=>t.file===f));
