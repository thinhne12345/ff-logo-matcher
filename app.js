...
function makePng(fileObj){
  return new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>{const canvas=document.createElement('canvas');canvas.width=1000;canvas.height=1000;const ctx=canvas.getContext('2d',{alpha:false});ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';const dark=ctx.createLinearGradient(0,0,1000,1000);dark.addColorStop(0,'#181d25');dark.addColorStop(.55,'#0f141c');dark.addColorStop(1,'#080c12');ctx.fillStyle=dark;ctx.fillRect(0,0,1000,1000);ctx.save();ctx.beginPath();ctx.arc(500,500,440,0,Math.PI*2);ctx.clip();const scale=Math.max(880/img.width,880/img.height),w=img.width*scale,h=img.height*scale;ctx.drawImage(img,(1000-w)/2,(1000-h)/2,w,h);ctx.restore();ctx.beginPath();ctx.arc(500,500,440,0,Math.PI*2);ctx.lineWidth=8;ctx.strokeStyle='#080b10';ctx.stroke();drawVietnamFlag(ctx);canvas.toBlob(blob=>blob?resolve(blob):reject(new Error('Không tạo được PNG')),'image/png')};img.onerror=reject;img.src=fileObj.url});
}
...
async function downloadOne(i){const t=teams[i];if(!t?.file)return;if(!HEADPICS.some(([,id])=>id===t.id))return alert('Vui lòng chọn HEADPICS cho team trước khi xuất ảnh.');save(await makePng(t.file),`${t.id}.png`)}
const csv=v=>/[",\n]/.test(String(v))?`"${String(v).replace(/"/g,'""')}"`:String(v);

async function downloadZip(){
  const matched=teams.filter(t=>t.file);if(!matched.length)return alert('Chưa có logo nào đã ghép.');const missing=matched.filter(t=>!HEADPICS.some(([,id])=>id===t.id));if(missing.length)return alert('Vui lòng chọn HEADPICS cho mọi team đã gắn logo trước khi tải ZIP.');$('zip').disabled=true;const entries=[];
  try{for(let i=0;i<matched.length;i++){const t=matched[i],blob=await makePng(t.file);entries.push({name:`${t.id}.png`,data:new Uint8Array(await blob.arrayBuffer())});$('bar').style.width=`${(i+1)/matched.length*100}%`}
  const map='NO,TEAM,HEADPICS_ID,ORIGINAL_FILE,SIZE,STYLE\n'+matched.map(t=>[t.no,t.team,t.id,t.file.name,'1000x1000','circle-red-corners-vietnam-flag'].map(csv).join(',')).join('\n');entries.push({name:'mapping.csv',data:new TextEncoder().encode(map)});save(buildZip(entries),'ff_team_avatars_1000x1000.zip')}finally{$('zip').disabled=false}
}
...
function renderHeadpics(){
  $('tbody').innerHTML=teams.map((t,i)=>`<tr><td>${esc(t.no)}</td><td><strong>${esc(t.team)}</strong></td><td>${t.file?`<div class="avatar-preview"><img class="preview" src="${t.file.url}" alt=""></div>`:`<label class="row-file">Ch&#7885;n logo<input type="file" accept="image/png,image/jpeg,image/webp" onchange="pickForTeam(${i},this.files[0])"></label>`}</td><td><select class="headpics-select" onchange="setHeadpics(${i},this.value)"><option value="">Ch&#7885;n HEADPICS...</option>${HEADPICS.map(([name,id])=>`<option value="${id}" ${t.id===id?'selected':''}>${name} &mdash; ${id}</option>`).join('')}</select></td><td>${t.file?`<span style="color:#67e66f">&#272;&#227; gh&#233;p t&#7921; &#273;&#7897;ng &middot; ${esc(t.avatar||t.id)}</span>`:'<span style="color:#f5c451">Ch&#432;a c&#243; logo</span>'}</td><td>${t.file?`<button onclick="downloadOne(${i})">T&#7843;i PNG</button> <button onclick="detach(${i})">B&#7887;</button>`:''}</td></tr>`).join('');
  const free=logos.filter(f=>!teams.some(t=>t.file===f));
  $('unmatched').innerHTML=free.map(f=>`<div class="card"><img src="${f.url}" alt="${esc(f.name)}"><strong class="filename" title="${esc(f.name)}">${esc(f.name)}</strong><select onchange="assign('${f.key}',this.value)"><option value="">Ch&#7885;n team...</option>${teams.map((t,i)=>`<option value="${i}">${esc(t.team)}</option>`).join('')}</select></div>`).join('')||'<p>Kh&#244;ng c&#243; logo ch&#7901; gh&#233;p.</p>';
  $('tc').textContent=teams.length;$('lc').textContent=logos.length;$('mc').textContent=teams.filter(t=>t.file).length;
}
